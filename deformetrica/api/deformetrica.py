...
class Deformetrica:
    ...
    def estimate_deterministic_atlas(self, template_specifications, dataset_specifications,
        ...
        """
        #old coarse to fine: after convergence
        if model_options['optimize_nb_control_points'] == True:
            
            print("______First classical optimization with few points..._____")
            model_options['optimize_nb_control_points'] = False

            #launch a classical GD with 8 points
            # Instantiate model.
            statistical_model = DeterministicAtlas(template_specifications, dataset.number_of_subjects, **model_options)
            statistical_model.initialize_noise_variance(dataset)
            statistical_model.setup_multiprocess_pool(dataset)
            ...
            #localize the residuals and then launch a coarse to fine
            residuals_difference = 10
            while residuals_difference > 0.01:
                print("_____Another optimization with coarse to fine...______")
                model_options['optimize_nb_control_points'] = True
                                
                statistical_model = DeterministicAtlas(template_specifications, dataset.number_of_subjects, **model_options)
                statistical_model.initialize_noise_variance(dataset)
                statistical_model.setup_multiprocess_pool(dataset)

                # Give the GD the last residuals
                estimator_options["final_residuals_ratio"] = estimator.final_residuals_ratio
                estimator_options["last_residuals"] = estimator.current_residuals
                estimator_options["initial_residuals"] = estimator.initial_residuals
                estimator = self.__instantiate_estimator(statistical_model, dataset, estimator_options, default=ScipyOptimize)
                ...
                #give to GD the last control points to initialize CTF
                estimator_options["last_control_points"] = estimator.last_control_points
                if len(estimator.final_residuals_ratio) > 1:
                    residuals_difference = np.abs(estimator.final_residuals_ratio[-1] - estimator.final_residuals_ratio[-2])
                if estimator.final_residuals_ratio[-1] < estimator.final_residuals_ratio[-2]:
                    residuals_difference = 0
            print("estimator.final_residuals_ratio", estimator.final_residuals_ratio)"""
        return statistical_model
...
##########
    ...
    def compute_shooting(self, template_specifications, model_options={}):
        ...
        # Launch.
        print("compute_shooting")
        print("template_specifications", template_specifications)
    ...
    def further_initialization(self, model_type, template_specifications, model_options,
        ...
        if 'multiscale_momenta' not in model_options: #ajout fg
            model_options['multiscale_momenta'] = default.multiscale_momenta
        if 'multiscale_images' not in model_options: #ajout fg
            model_options['multiscale_images'] = default.multiscale_images
        if "gamma" not in model_options:
            model_options['gamma'] = default.gamma
        if 'max_spacing' not in model_options: #ajout fg
            model_options['max_spacing'] = default.max_spacing
        ...
        if 'initial_cp_spacing' not in model_options:
            print('initial_cp_spacing not in model_options')
        ...
        if model_type.lower() in ['Regression'.lower(), 'ClusteredLongitudinalAtlas'.lower(),
        ...
        if estimator_options is not None and \
            ...
            assert model_type.lower() in ['LongitudinalAtlas'.lower(), 'BayesianAtlas'.lower(), 'ClusteredLongitudinalAtlas'.lower(), 'ClusteredBayesianAtlas'.lower(), 'BayesianAtlasSparse'.lower()], \
