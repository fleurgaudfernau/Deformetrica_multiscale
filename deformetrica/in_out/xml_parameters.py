...
def get_estimator_options(xml_parameters):
    ...
    if xml_parameters.optimization_method_type.lower() in ['GradientAscent'.lower(), 'ProximalGradientAscent'.lower()]:
...
def get_model_options(xml_parameters):
    options = {
        ...
        'multiscale_momenta': xml_parameters.multiscale_momenta, #ajout fg
        'multiscale_images': xml_parameters.multiscale_images,
        'naive': xml_parameters.naive, #ajout fg
        "max_spacing" : xml_parameters.max_spacing, #ajout fg
        "gamma" : xml_parameters.gamma, #ajout fg
    ...
    if xml_parameters.model_type.lower() in ['LongitudinalAtlas'.lower(), 'LongitudinalRegistration'.lower(), 'ClusteredLongitudinalAtlas'.lower()]:
...
class XmlParameters:
    ...
    def __init__(self):
        ...
        self.max_spacing = default.max_spacing #ajout fg
    ...
    def _read_optimization_parameters_xml(self, optimization_parameters_xml_path):
        if optimization_parameters_xml_path is not None and optimization_parameters_xml_path != 'None':
            ...
            for optimization_parameters_xml_level1 in optimization_parameters_xml_level0:
                ...
                elif optimization_parameters_xml_level1.tag.lower() == 'max-spacing': #ajout fg
                    self.max_spacing = float(optimization_parameters_xml_level1.text)
