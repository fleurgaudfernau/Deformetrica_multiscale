...
class BayesianAtlasSparse(AbstractStatisticalModel):
    ...
    def __init__(self, template_specifications, nb_subjects,
        ...
        self.exponential_t = Exponential(
            dense_mode=dense_mode,
            kernel=kernel_factory.factory(deformation_kernel_type, gpu_mode=gpu_mode,
        ...
        smoothing = GaussianSmoothing(1, 1, self.gaussian_smoothing_var)
        shape = image.shape
        ...
        self.individual_random_effects['sparse_matrix'] = LaplaceDistribution()
        self.individual_random_effects['sparse_matrix'].mean = np.zeros(self.get_template_data()['image_intensities'].shape)
        self.individual_random_effects['sparse_matrix'].scale = 100000
    ...
    def initialize_random_effects_realization(
        ...
        individual_RER = {
            'momenta': initialize_momenta(initial_momenta, self.number_of_control_points, self.dimension,
                                          number_of_subjects),
            'sparse_matrix': np.zeros(np.concatenate((number_of_subjects*np.ones(1),self.get_template_data()['image_intensities'].shape)).astype('int'))
        }
    ...
    def initialize_noise_variance(self, dataset, individual_RER):
        ...
        momenta, sparse_matrix = self._individual_RER_to_torch_tensors(individual_RER, False)
    ...
    def set_sparse_matrix(self, w):
        self.fixed_effects['sparse_matrix'] = w
    ...
    def compute_log_likelihood(self, dataset, population_RER, individual_RER, mode='complete', with_grad=False,
        ...
        momenta, sparse_matrix = self._individual_RER_to_torch_tensors(
        ...
        if mode == 'complete':
            regularity = self._compute_random_effects_regularity(momenta, sparse_matrix)
    ...
    def update_fixed_effects(self, dataset, sufficient_statistics):
        ...
        #for i in range(dataset.number_of_subjects):
        #    self.individual_random_effects['sparse_matrix_subj' + str(i)].set_image_grad(sufficient_statistics['S5'][i])
    ...
    def _compute_random_effects_regularity(self, momenta, control_points, sparse_matrix):
        """
        Fully torch.
        """
    ...
    def _individual_RER_to_torch_tensors(self, individual_RER, with_grad):
        ...
        sparse_matrix = individual_RER['sparse_matrix']
        sparse_matrix = torch.from_numpy(sparse_matrix).type(self.tensor_scalar_type).requires_grad_(with_grad)

        return momenta, sparse_matrix
    ...
    def _write_model_parameters(self, individual_RER, output_dir):
        ...
        write_3D_array(individual_RER['sparse_matrix'], output_dir, self.name + "__EstimatedParameters__SparseMatrix.txt")
