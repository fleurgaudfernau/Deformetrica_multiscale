...
from ...support.probability_distributions.multi_scalar_normal_distribution_clustered import MultiScalarNormalDistributionClustered
...
class BayesianAtlasSparse(AbstractStatisticalModel):
    ...
    def __init__(self, template_specifications,
        ...
        self.fixed_effects['intensity_classes'] = None
        ...
        self.priors['module_intensities'] = MultiScalarNormalDistributionClustered()
        ...
        self.individual_random_effects['module_intensities'] = MultiScalarNormalDistributionClustered()
        ...
        self.individual_random_effects['module_intensities'].set_variance(5)
        for i in range(10):
            for k in range(self.number_of_modules):
                self.individual_random_effects['module_positions' + str(k) + '_subj' + str(i)] = AlamainDistribution()
                self.individual_random_effects['module_positions' + str(k) + '_subj' + str(i)].cp = initial_cp
                self.individual_random_effects['module_positions' + str(k) + '_subj' + str(i)].set_variance(50)
                self.individual_random_effects['module_positions' + str(k) + '_subj' + str(i)].box = self.template.bounding_box
        self.individual_random_effects['module_variances'].mean = np.array(0.)
        self.individual_random_effects['module_variances'].set_variance(100)

        self.priors['module_intensities'].set_mean(np.array([-0.1,0.1]))
        self.priors['module_intensities'].set_variance_sqrt(100.)
    ...
    def initialize_random_effects_realization(
        ...
        individual_RER = {
            'momenta': initialize_momenta(initial_momenta, self.number_of_control_points, self.dimension,
                                          number_of_subjects),
            'module_intensities': np.zeros([number_of_subjects,self.number_of_modules]),
            'module_variances': 5*np.ones([number_of_subjects,self.number_of_modules,self.dimension]),
            'intensity_classes': np.random.randint(0,2,number_of_subjects)
        }
        for i in range(number_of_subjects):
            for k in range(self.number_of_modules):
                individual_RER['module_positions' + str(k) + '_subj' + str(i)] = np.array(self.template.get_points()['image_points'].shape[:-1])/2
    ...
    def initialize_noise_variance(self, dataset, individual_RER):
        ...
        momenta, module_positions, module_intensities, module_variances = self._individual_RER_to_torch_tensors(individual_RER, False)
    ...
    def compute_log_likelihood(self, dataset, population_RER, individual_RER, mode='complete', with_grad=False,
        ...
        momenta, module_positions, module_intensities, module_variances = self._individual_RER_to_torch_tensors(
            individual_RER, False)
        sparse_matrix = self.construct_sparse_matrix(template_points['image_points'], module_positions,
                                                     module_variances, module_intensities)
        ...
        residuals = self._compute_residuals(dataset, template_data, template_points, control_points, momenta, sparse_matrix)
    ...
    def compute_sufficient_statistics(self, dataset, population_RER, individual_RER, residuals=None, model_terms=None):
        ...
        sparse_matrix = self.construct_sparse_matrix(template_points['image_points'],
                                                     module_positions, module_variances, module_intensities)
        ...
        for i in range(dataset.number_of_subjects):
            for k in range(self.number_of_modules):
                self.individual_random_effects['module_positions' + str(k) + '_subj' + str(i)].set_dir(
                    momenta_t[i])
    ...
    def update_fixed_effects(self, dataset, sufficient_statistics):
        ...
        for i in range(dataset.number_of_subjects):
            for k in range(self.number_of_modules):
                self.individual_random_effects['module_positions' + str(k) + '_subj' + str(i)].set_cp(sufficient_statistics['S5'][i])
    ...
    def _individual_RER_to_torch_tensors(self, individual_RER, with_grad):
        ...
        module_positions = np.zeros([momenta.shape[0], self.number_of_modules, individual_RER['module_positions0_subj0'].size])
        for i in range(momenta.shape[0]):
            for k in range(self.number_of_modules):
                module_positions[i,k] = individual_RER['module_positions' + str(k) + '_subj' + str(i)]
        module_positions = torch.from_numpy(np.array(module_positions)).type(self.tensor_scalar_type).requires_grad_(with_grad)
        module_intensities = individual_RER['module_intensities']
        module_intensities = torch.from_numpy(module_intensities).type(self.tensor_scalar_type).requires_grad_(with_grad)
        module_variances = individual_RER['module_variances']
        module_variances = torch.from_numpy(module_variances).type(self.tensor_scalar_type).requires_grad_(with_grad)
        return momenta, module_positions, module_intensities, module_variances

    def construct_sparse_matrix(self, points, module_centers, module_variances, module_weights):
        dim = (module_weights.shape[0],) + self.fixed_effects['template_data']['image_intensities'].shape
        ...
        for i in range(dim[0]):
            for k in range(self.number_of_modules):
                ...
                sparse_matrix[i] += torch.exp(-dist)*module_weights[i,k]
    ...
    def _write_model_parameters(self, individual_RER, output_dir):
        ...
        positions = []
        for i in range(individual_RER['momenta'].shape[0]):
            positions.append([])
            for k in range(self.number_of_modules):
                positions[i].append(individual_RER['module_positions' + str(k) + '_subj' + str(i)])
        write_3D_array(np.array(positions), output_dir, self.name + "__EstimatedParameters__ModulePositions.txt")
        write_3D_array(individual_RER['module_intensities'], output_dir, self.name + "__EstimatedParameters__ModuleIntensities.txt")
        write_3D_array(individual_RER['module_variances'], output_dir, self.name + "__EstimatedParameters__ModuleVariances.txt")
